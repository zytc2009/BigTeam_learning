...
git config --global user.name "wanghongbin"
...
git diff 版本哈希值 版本哈希值 // 查看这2个版本哈希之间的区别或者 
git diff HEAD~数字 HEAD~数字
...
git checkout commitId 文件名（文件路径下的文件名） 还原这个文件到对应的commitId的版本（例如src/page/attendance/attendanceSum.java我想把它还原到2个版本之前 首先git log src/attendance/attendanceSum.java找到对应想要还原的版本复制版本提交的commitID 然后执行git checkout commitID src/attendance/attendanceSum.java这样就把attendanceSum.java这个单个文件 还原到了对应版本）
